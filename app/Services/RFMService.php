...
class RFMService
{
    ...
    public static function rfm($subQuery, $rfmPrms)
    {
     ...
     // 2. 会員毎にまとめて最終購入日、回数、合計金額を取得(datediffで日付の差分, maxで日付の最新日)
     $subQuery = DB::table($subQuery)
         ->groupBy('customer_id')
         ->selectRaw('customer_id, customer_name, max(created_at) as recentDate,
             datediff(now(), max(created_at)) as recency, count(customer_id) as frequency, sum(totalPerPurchase) as monetary');
